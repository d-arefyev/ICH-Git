...
router.post('/auth', async (req, res) => {
    try {
        ...
        // Исправление 1: добавил проверку на существование пользователя и return
        if (!user) {
        ...
        // Исправление 2: если пароль неверный, тоже добавил return
        if (!isMatch) {
            return res.status(401).send('Пароль неверный');
        }

        const token = jwt.sign(
            { userId: user.idUser, email: user.userName }, 
            '123123123', 
            { expiresIn: '1h' }
        );
...
router.post('/register', async (req, res) => {
    try {
        ...
        // Исправление 3: добавил return, если пользователь уже существует
        if (user) {
        ...
        // Исправление 4: исправил возвращение id нового пользователя
        const token = jwt.sign(
            { userId: newUser.idUser, email: newUser.userName },
            '123123123',
            { expiresIn: '1h' }
        );

        return res.json({ token, id: newUser.idUser }); // Использовал newUser.idUser
